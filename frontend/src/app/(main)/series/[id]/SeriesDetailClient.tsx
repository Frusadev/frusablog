...
export default function SeriesDetailClient({ seriesId }: SeriesDetailClientProps) {
  const { data: series, isLoading: seriesLoading } = useQuery({
    queryKey: ["series", seriesId],
    queryFn: () => getPostSeries(seriesId),
    enabled: !!seriesId,
  });
  ...
  const {
    ...
    queryKey: ["series-posts", seriesId, PAGE_SIZE],
    initialPageParam: 0,
    queryFn: ({ pageParam }) => getSeriesPosts(seriesId, pageParam as number, PAGE_SIZE),
